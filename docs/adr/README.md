...
| [0009](0009-scale-assumptions-and-derived-decisions.md) | 규모 가정과 그로부터 파생된 구조 결정 (사후 기록) | Accepted | 2026-09-04 |
...
ADR-0009 (규모 가정과 파생 결정) — 위 결정들이 고른 구성값(커넥션 풀 20 · 컨슈머 병렬도 3 ·
  낙관락 선점 · 스위퍼 4종 · append-only 원장)이 어떤 규모를 전제하는지 역산하고, 각 값이
  뒤집히는 조건을 명시한다. ADR-0006·0007 의 "지금은 분해/분리하지 않는다" 와 같은 근거(현재 규모)를 공유.
```
