package com.carry_laundry.carry_backend.order.repository.custom.impl;
...
public class OrderCustomRepositoryImpl implements OrderCustomRepository {
    ...
    public Mono<Order> getOrderDetail(Long orderId, Long userId) {
        ...
        DatabaseClient.GenericExecuteSpec spec = r2dbcEntityTemplate.getDatabaseClient()
            .sql(selectSQL)
    ...
    private Order mapToObject(String baseJson, String specsJson, String optionsJson,
        ...
        try {
            ...
            if (specsJson != null) {
                List<OrderSpecification> specs = objectMapper.readValue(specsJson,
                    new TypeReference<>() {
                    });
                order.setOrderSpecifications(specs);
            }
            if (optionsJson != null) {
                List<OrderOption> options = objectMapper.readValue(optionsJson,
                    new TypeReference<>() {
                    });
                order.setOrderOptions(options);
            }
            if (addressJson != null) {
                OrderShippingAddress address = objectMapper.readValue(addressJson,
                    new TypeReference<>() {
                    });
            ...
            if (invoiceJson != null) {
                ...
                if (chargesJson != null) {
                    List<InvoiceCharge> charges = objectMapper.readValue(chargesJson,
                        new TypeReference<>() {
                        });
