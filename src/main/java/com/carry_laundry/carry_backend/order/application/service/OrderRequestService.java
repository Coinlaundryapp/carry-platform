...
public class OrderRequestService {
    ...
    public Mono<Tuple2<Order, Invoice>> createOrder(CreateOrderRequest request, Long userId) {
        ...
        return Mono.zip(laundromatService.getLaundromatById(request.getLaundromatId()),
            ...
            .flatMap(tuple3 -> {
                ...
                OrderShippingAddress orderShippingAddress = OrderShippingAddress.create(order,
                    shippingAddress);
                return r2dbcEntityTemplate.insert(OrderShippingAddress.class)
                    .using(orderShippingAddress)
                    .thenReturn(tuple3);
            })
            .flatMap((Tuple3<Order, Laundromat, ShippingAddress> tuple3) -> {
                Order order = tuple3.getT1();
                Flux<OrderSpecification> specFlux = Flux.fromIterable(
                    convertSpec(order.getId(), request.getOrderContent()));
                return specFlux.flatMap(
                        orderSpec -> r2dbcEntityTemplate.insert(OrderSpecification.class)
                            .using(orderSpec))
                    .then().thenReturn(tuple3);
            })
            .flatMap((Tuple3<Order, Laundromat, ShippingAddress> tuple3) -> {
                Order order = tuple3.getT1();
                Flux<OrderOption> optionFlux = Flux.fromIterable(
                    convertOption(order.getId(), request.getOrderContent()));
                return optionFlux.flatMap(
                        orderOption -> r2dbcEntityTemplate.insert(OrderOption.class).using(orderOption))
                    .then().thenReturn(tuple3);
            })
            .flatMap((Tuple3<Order, Laundromat, ShippingAddress> tuple3) -> {
                Order order = tuple3.getT1();
                ...
                Invoice invoice = Invoice.create(order.getId(),
                    laundryPrice + serviceFee + deliveryFee, 0);
            ...
            .flatMap(tuple3 -> {
                ...
                Flux<InvoiceCharge> invoiceChargeFlux = Flux.fromIterable(
                    convertInvoiceCharge(invoice.getId(), laundryPrice, deliveryFee, serviceFee));
                return invoiceChargeFlux.flatMap(
                        invoiceCharge -> r2dbcEntityTemplate.insert(InvoiceCharge.class)
                            .using(invoiceCharge))
    ...
    private List<OrderSpecification> convertSpec(Long orderId, OrderContent content) {
        return content.laundrySpecs().stream()
            .map(specDescription -> OrderSpecification.create(orderId, specDescription)).toList();
    }

    private List<InvoiceCharge> convertInvoiceCharge(Long invoiceId, int laundryPrice,
        int deliveryFee, int serviceFee) {
