package com.carry_laundry.carry_backend.geo.external.recrod;
...
public record GetGeocodingResponse(
    ...
    public record Address(
        ...
        public Address(
            ...
            this(roadAddress, jibunAddress, englishAddress, getAddressElementsMap(addressElements),
                x, y, distance);
        }

        private static Map<AddressType, AddressElement> getAddressElementsMap(
            List<AddressElement> addressElements) {
        ...
        public String getAddressElementNameOrNull(AddressType type) {
            AddressElement element = addressElementMap.getOrDefault(type,
                AddressElement.emptyAddressElementsMap.get(type));
            if (element.isEmpty()) {
                return null;
            }
        ...
        private static AddressType getType(List<AddressType> typeList) {
            ...
            if (typeList.isEmpty()) {
                throw new IllegalStateException(
                    "AddressElement의 types가 비어있습니다. naver api의 응답이 변경되었을 수 있습니다.");
            }
            return typeList.get(0);
        }

        static final Map<AddressType, AddressElement> emptyAddressElementsMap = new EnumMap<>(
            AddressType.class) {
            {
